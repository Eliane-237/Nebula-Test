...
import { ArrowUpRight, CalendarDays, CheckCircle2, Clock3, Compass } from 'lucide-react';
...
type Program = { id: string; title: string; domain: string; numSessions: number; explorations: { id: string; title: string; description: string }[] };
type Cohort  = { startDate: Date | string; endDate: Date | string; sessions: Session[] };
        ...
        <div className="myprog-explo-section">
          ...
          <div className="myprog-explo-grid">
            {allExplorations.map((e) => (
              <div key={e.id} className="myprog-explo-card">
                <div className="myprog-explo-label">{e.programTitle}</div>
                <div className="myprog-explo-title">{e.title}</div>
                <div className="myprog-explo-desc">{e.description}</div>
                <div className="myprog-explo-footer">
                  <span className="myprog-due"><Clock3 size={11} /> Pending</span>
                  <button className="myprog-explo-btn">Start <ArrowUpRight size={10} /></button>
                </div>
              </div>
            ))}
          </div>
